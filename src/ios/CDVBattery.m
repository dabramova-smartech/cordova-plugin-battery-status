...
- (void)getBatteryLevel:(CDVInvokedUrlCommand*)command {
    ...
    BOOL isMonitored = device.batteryMonitoringEnabled;
    if (!isMonitored)
        device.batteryMonitoringEnabled = YES;

    int level = (int)device.batteryLevel * 100;
    device.batteryMonitoringEnabled = isMonitored;

    CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:level];
